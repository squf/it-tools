...
* You would have to set-up [Data Connect](https://learn.microsoft.com/en-us/graph/data-connect-concept-overview) to avoid all API limits to my understanding, and I might have to go this route actually. This won't change much in the scripts here, since I would just update the clientID and clientSecret in `get-token.ps1` and delete the other one I made, or, maybe even keep the same app details I set-up prior and just link it into MGDC.
