...
# Connect to any HTTPS site — Palo Alto intercepts and presents its own chain# We grab ALL certs in the chain, then take the LAST one (the root CA)
echo | openssl s_client -connect raw.githubusercontent.com:443 -showcerts 2>/dev/null | \  awk '/-----BEGIN CERTIFICATE-----/{block=""}        {block=block $0 "\n"}        /-----END CERTIFICATE-----/{last=block}        END{printf "%s", last}' > /tmp/palo-alto-root-ca.crtShow more lines
...
sudo cp /tmp/palo-alto-root-ca.crt /usr/local/share/ca-certificates/palo-alto-root-ca.crtsudo update-ca-certificatesShow more lines

Step 4: Test
# The moment of truth
curl -v https://raw.githubusercontent.com 2>&1 | head -30Show more lines
