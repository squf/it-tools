...
This repository contains several scripts, but **only `ODHF.ps1` is needed for production use**. The others are included for context and archival purposes.
  ...
  * **`make-homefolder-od.ps1`**: Later we did decide to just purchase ShareGate for our tenant, however we wanted to keep the logic of moving home folders from our onprem share to a user's OneDrive -> into a directory called "Home Folder". This script is a pared down version of the beefy `ODHF.ps1` -- this script ONLY makes a "Home Folder" at the user's OneDrive root, so that when you are migrating files via ShareGate you can just select this folder (it looks a lot like an FTP upload) and then dump everything into here to keep it neat & tidy. In other words there is no need for this script if you are going to wing it with `ODHF.ps1`, which already includes logic for making the folder and performing the full migration. This script `make-homefolder-od.ps1` is only useful in our context at my current company since we purchased ShareGate and still had a need to have the folders present sans migrated data (since we will just use ShareGate to do the migration, savvy?).
